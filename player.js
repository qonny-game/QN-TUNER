(() => {
  ...
  // Sensitivity: 0-100 slider value.
  ...
  const HOLD_FRAMES_MAX = 240;     // ~4s hold at sensitivity = 100

  function silenceRmsThreshold() {
    const t = sensitivity / 100;
    return SILENCE_RMS_MAX + (SILENCE_RMS_MIN - SILENCE_RMS_MAX) * t;
  }
  ...
  function autoCorrelate(buf, sampleRate) {
    ...
    if (rms < silenceRmsThreshold()) return -1; // treat as silence
  ...
  function tickMic() {
    ...
    if (freq === -1 || freq < 30 || freq > 2000) {
      micSilenceFrames++;
      if (micSilenceFrames === silenceHoldFrames()) {
        noteDisplay.textContent = '—';
        freqValue.textContent = '0.0';
        centsDisplay.textContent = 'Play a note';
        needle.style.transform = 'rotate(0deg)';
